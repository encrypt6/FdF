...
void	draw(t_map *map_cpy, t_data data)
{
	...
	while (i < map_cpy->height)
	{
		...
		while (j < map_cpy->width)
		{
			...
			iso_points = isometric(points[i][j], map_cpy->width, map_cpy->height);
			...
			{
				next_iso_points = isometric(points[i][j + 1], map_cpy->width, map_cpy->height);
				ft_xiaolin_wu(iso_points, next_iso_points, data);
			}
			if (i < map_cpy->height- 1)
			{
				below_iso_points = isometric(points[i + 1][j], map_cpy->width, map_cpy->height);
